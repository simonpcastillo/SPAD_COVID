...
plot_relations <- function(CPAD, saveplot=TRUE, saveplot.ext= ".png"){
  ...
  G = ggplot(df4, aes(log(beta),countries,colour=as.numeric(day))) +
    geom_point(aes(group = seq_along(day)), size=2)+
    #geom_path(size=1.5, alpha=0.3)+
    labs(x= expression(paste("ln(",beta, ")")), y= expression("# Countries"), title= "B. ") +
    theme_minimal() +
  ...
  FFinset = ggplot(df4, aes((alfa), NcasesPos,colour=as.numeric(day))) +
    ...
    labs(x= expression(paste(alpha)), y= expression(paste("# SARS-Cov-2"))) +
    theme_minimal() +
  ...
  Ginset=ggplot(df4, aes((beta),countries,colour=as.numeric(day))) +
    geom_path(size=1, colour="gray60")+
    labs(x= expression(paste(beta)), y= expression(paste("# Countries"))) +
    theme_minimal() +
  ...
  plotrelations <<- list.append(plotrelations, alphaSARS= FF2, betaCountries=G2)
  ...
  if(saveplot==TRUE){
    ...
    ggsave(plot = FF2|G2, filename = paste("plots/betaplots/relations", saveplot.ext), width = 25, height = 12, units = "cm")
  }
