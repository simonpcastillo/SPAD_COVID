...
plot_betatraj <- function(CPAD, saveplot=TRUE, saveplot.ext= ".png"){
  ...
  A =ggplot(df4, aes(log(alfa), log(beta),colour=as.numeric(day))) +
  ...
  B=ggplot(df4, aes(day, log(alfa), colour=as.numeric(day))) + #log(alfa)
    geom_point(aes(group = seq_along(day)), size=2)+
    #geom_path(size=1.5, alpha=0.6)+
    labs(x= "", y= expression(paste("ln(",alpha, ")"))) +
    theme_minimal() +
    guides(colour=FALSE)+
    theme(axis.title=element_text(size=12,face="bold"))+
    scale_colour_viridis_c(breaks = as.numeric(breaksdate),
                           ...
                           name = "Day",
                           option = "plasma")
  C=ggplot(df4, aes(day, log(beta), colour=as.numeric(day))) +
    geom_point(aes(group = seq_along(day)), size=2)+
    #geom_path(size=1.5, alpha=0.3)+
    labs(x= "", y= expression(paste("ln(",beta, ")"))) +
    theme_minimal() +
    guides(colour=FALSE)+
    theme(axis.title=element_text(size=12,face="bold"))+
    scale_colour_viridis_c(breaks = as.numeric(breaksdate),
  ...
  D=ggplot(df4, aes(day, log(E_beta), colour=as.numeric(day))) +
    geom_point(aes(group = seq_along(day)), size=2)+
    #geom_path(size=1.5, alpha=0.3)+
    labs(x= "", y= expression("ln(E|prop ab|)")) +
    theme_minimal() +
    guides(colour=FALSE)+
    theme(axis.title=element_text(size=12,face="bold"))+
    scale_colour_viridis_c(breaks = as.numeric(breaksdate),
  ...
  E=ggplot(df4, aes(day, log(V_beta), colour=as.numeric(day))) +
    geom_point(aes(group = seq_along(day)), size=2)+
    #geom_path(size=1.5, alpha=0.3)+
    labs(x= "", y= expression("ln(Var|prop ab|)")) +
    theme_minimal() +
    #guides(colour=FALSE)+
    theme(axis.title=element_text(size=12,face="bold"))+
    scale_colour_viridis_c(breaks = as.numeric(breaksdate),
