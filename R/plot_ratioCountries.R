...
#'
#'
plot_ratioCountries <- function(CPAD,disease ,saveplot=TRUE, saveplot.ext= ".png"){
 ...
 # H = ggplot(df4, aes(ratio_countries, (MRcasesPos), colour=as.numeric(day))) +
  ...
  I = ggplot(df4, aes(ratio_countries, (NcasesPos), colour=as.numeric(day))) +
    geom_point(aes(group = seq_along(day)), size=2)+
    #geom_path(size=1.5, alpha=0.3)+
    labs(x= expression("Ratio countries"), y= (paste0("#", disease)), title="B. ") +
    theme_minimal() +
    #guides(colour=FALSE)+
    theme(axis.title=element_text(size=12,face="bold"))+
    scale_colour_viridis_c(breaks = as.numeric(breaksdate),
  ...
  J = ggplot(df4, aes(ratio_countries, (McasesPos), colour=as.numeric(day))) +
    geom_point(aes(group = seq_along(day)), size=2)+
    #geom_path(size=1.5, alpha=0.3)+
    labs(x= expression("Ratio countries"), y= (paste0("E|#",disease,"|")), title = "C. ") +
    theme_minimal() +
    guides(colour=FALSE)+
    theme(axis.title=element_text(size=12,face="bold"))+
    scale_colour_viridis_c(breaks = as.numeric(breaksdate),
  ...
  plotratioCountries <<- list.append(plotratioCountries, countriesNCases=I)
  ...
  if(saveplot==TRUE){
    ...
    ggsave(plot = I/J, filename = paste("plots/betaplots/plotratioCountries_",disease, saveplot.ext), width = 25, height = 18, units = "cm")
  }
